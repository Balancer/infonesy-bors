...
class Object
{
	...
	function table_name()
	{
		$name = strtolower(str_replace('B2\\MyBB\\', '', get_called_class())); // '

		if(preg_match('/^(.+)y$/', $name, $m))
			return 'mybb_'.$m[1].'ies';
	...
	function __construct($object_id)
	{
		...
		if($object_id)
		{
			$this->data_map = $this->mapper->$table_name($object_id)->fetch();

			if(!$this->data_map)
			{
//				throw new \Exception("Can't find {$table_name}({$object_id})");
...
//		    $this->data_map->id = NULL;
	...
	function save()
	{
		$table_name = $this->table_name();
//		$id = $this->id;
		$this->mapper->$table_name->persist($this->data_map);
//		echo "Flush for ".get_class($this)."\n";
//		print_r(debug_backtrace());
		$this->mapper->flush();
//		$this->data_map = $this->mapper->$table_name($id)->fetch();
	}
}
